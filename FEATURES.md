...
| Careers / Our Team Directory | Implemented | Public-facing team directory (`CareersPage.tsx`, `/careers`); live Supabase query (`publicStaffService.ts`) selecting safe staff & clinic fields; multi-attribute search, role/designation/clinic filters, initials fallback avatar, responsive grid inspired by Labaid doctor directory, pagination, error/empty states, integrated into footer (`AppFooter.tsx`) |
