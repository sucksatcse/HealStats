...
- **Admin map (2026-09-08)**: Leaflet map has a minimum 360px height, keyboard-accessible markers/search/zoom controls and numeric coordinate inputs as an alternative to click-to-place. The directory stacks and collapses below 768px; the missing-coordinate list remains accessible there. Marker status includes labels, not just color. Use OSM standard light tiles and CARTO dark tiles rather than CSS inversion. Keep visible attribution away from the chatbot and constrain popup height. Respect reduced-motion when flying to a result; do not prefetch tiles or claim offline tile availability. External place search is explicit-submit only with a privacy disclosure; local clinic filtering can update while typing.
