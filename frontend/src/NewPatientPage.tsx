...
const INITIAL: FormData = {
  fullName: "", dob: "", age: "", sex: "", village: "", phone: "",
  emergencyName: "", emergencyPhone: "", emergencyRelation: "",
  bloodType: "", allergies: "", medications: "", conditions: [], pregnancyStatus: "",
  vaccinations: "",
};
...
const VILLAGES = ["Kayes Centre", "Dialafara", "Sandaré", "Bafoulabé", "Kéniéba", "Mahina", "Toukoto", "Diéma", "Yélimané", "Nioro du Sahel"];
const RELATIONS = ["Spouse", "Parent", "Child", "Sibling", "Friend", "Other"];
const BLOOD_TYPES = ["A+", "A−", "B+", "B−", "AB+", "AB−", "O+", "O−", "Unknown"];
const CHRONIC_CONDITIONS = ["Hypertension", "Type 2 Diabetes", "Asthma", "HIV/AIDS", "Tuberculosis", "Sickle Cell Disease", "Malaria (recurrent)", "Epilepsy", "Malnutrition"];
...
function FieldInput({ id, type = "text", placeholder, value, onChange, error }: {
...
function FieldSelect({ id, value, onChange, children, error }: {
  ...
  return (
    <div>
      <select
        ...
        style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M4 6l4 4 4-4' stroke='%2394a3b8' stroke-width='1.6' stroke-linecap='round'/%3E%3C/svg%3E")`, backgroundPosition: "right 12px center", backgroundSize: "16px" }}
      >
        {children}
      </select>
      {error && <p className="text-[11px] text-red-500 mt-1 ml-1">{error}</p>}
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-5 h-5 rounded-md bg-teal-100 flex items-center justify-center">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-3 h-3 text-teal-600">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 7a3 3 0 100-6 3 3 0 000 6zM2 14a6 6 0 0112 0" />
            </svg>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
          <div className="sm:col-span-2">
            ...
            <FieldInput
              id="fullName" placeholder="e.g. Mariama Kouyaté"
          ...
          <div>
            ...
            <FieldInput
              id="dob" type="date"
              value={data.dob} onChange={(v) => update("dob", v)}
              error={errors.dob}
            />
          </div>
          <div>
            ...
            <FieldInput
              id="age" type="number" placeholder="e.g. 34"
          ...
          <div>
            ...
            <FieldSelect id="sex" value={data.sex} onChange={(v) => update("sex", v)} error={errors.sex}>
          ...
          <div>
            ...
            <FieldSelect id="village" value={data.village} onChange={(v) => update("village", v)} error={errors.village}>
              <option value="" disabled>Select village or zone</option>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-5 h-5 rounded-md bg-teal-100 flex items-center justify-center">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-3 h-3 text-teal-600">
              <path strokeLinecap="round" strokeLinejoin="round" d="M2 3h12a1 1 0 011 1v8a1 1 0 01-1 1H2a1 1 0 01-1-1V4a1 1 0 011-1zm0 0l6 5 6-5" />
            </svg>
          </div>
          <h3 className="text-sm font-semibold text-slate-700">Contact</h3>
          <span className="text-[10px] text-slate-400 font-medium">(Phone optional)</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
          <div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-5 h-5 rounded-md bg-red-50 flex items-center justify-center">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-3 h-3 text-red-500">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 2v4m0 4h.01M2.93 13.07A8 8 0 1113.07 2.93 8 8 0 012.93 13.07z" />
            </svg>
          </div>
          <h3 className="text-sm font-semibold text-slate-700">Emergency Contact</h3>
          <span className="text-[10px] text-red-400 font-semibold uppercase tracking-wide bg-red-50 px-1.5 py-0.5 rounded">Required</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
          <div>
            <Label required>Contact Full Name</Label>
            <FieldInput
              id="emergencyName" placeholder="e.g. Amadou Kouyaté"
              value={data.emergencyName} onChange={(v) => update("emergencyName", v)}
              error={errors.emergencyName}
            />
          </div>
          <div>
            <Label required>Relationship</Label>
            <FieldSelect id="emergencyRelation" value={data.emergencyRelation} onChange={(v) => update("emergencyRelation", v)} error={errors.emergencyRelation}>
          ...
          <div>
            ...
            {errors.emergencyPhone && <p className="text-[11px] text-red-500 mt-1 ml-1">{errors.emergencyPhone}</p>}
          </div>
        </div>
      </div>
    </div>
...
function Step2({ data, update, errors }: {
  ...
  const toggleCondition = (c: string) => {
    ...
    update("conditions", cur.includes(c) ? cur.filter((x) => x !== c) : [...cur, c]);
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-5 h-5 rounded-md bg-teal-100 flex items-center justify-center">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-3 h-3 text-teal-600">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 3.5v9M4.5 7h7M2 5.5A3.5 3.5 0 015.5 2h5A3.5 3.5 0 0114 5.5v5a3.5 3.5 0 01-3.5 3.5h-5A3.5 3.5 0 012 10.5v-5z" />
            </svg>
          </div>
          <h3 className="text-sm font-semibold text-slate-700">Clinical Basics</h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
          <div>
            <Label>Blood Type</Label>
            <FieldSelect id="bloodType" value={data.bloodType} onChange={(v) => update("bloodType", v)}>
          ...
          <div>
            ...
            <FieldSelect id="pregnancyStatus" value={data.pregnancyStatus} onChange={(v) => update("pregnancyStatus", v)}>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-5 h-5 rounded-md bg-amber-50 flex items-center justify-center">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-3 h-3 text-amber-500">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 2v4m0 4h.01M13.66 13.66A8 8 0 112.34 2.34a8 8 0 0111.32 11.32z" />
            </svg>
          </div>
          <h3 className="text-sm font-semibold text-slate-700">Known Allergies</h3>
        </div>
        <div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-5 h-5 rounded-md bg-teal-100 flex items-center justify-center">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-3 h-3 text-teal-600">
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 2h4l1 3H5L6 2zM3 5h10l-1 9H4L3 5z" />
            </svg>
          </div>
          <h3 className="text-sm font-semibold text-slate-700">Current Medications</h3>
        </div>
        <div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-5 h-5 rounded-md bg-violet-50 flex items-center justify-center">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-3 h-3 text-violet-500">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 14A6 6 0 108 2a6 6 0 000 12zM8 5v3.5l2 2" />
            </svg>
          </div>
          <h3 className="text-sm font-semibold text-slate-700">Chronic Conditions</h3>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-5 h-5 rounded-md bg-emerald-50 flex items-center justify-center">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-3 h-3 text-emerald-600">
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 8l3 3 5-5" />
              <circle cx="8" cy="8" r="6.5" />
            </svg>
          </div>
          <h3 className="text-sm font-semibold text-slate-700">Vaccination Notes</h3>
        </div>
        <div>
...
function Step3({ data }: { data: FormData }) {
  const ReviewField = ({ label, value, span }: { label: string; value: string; span?: boolean }) => (
    <div className={span ? "sm:col-span-2" : ""}>
      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-0.5">{label}</p>
      <p className="text-sm font-medium text-slate-800">{value || <span className="text-slate-400 italic font-normal">Not provided</span>}</p>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-50 rounded-2xl border border-slate-100 p-5">
        <p className="text-xs font-bold uppercase tracking-widest text-teal-600 mb-4">Personal Information</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
          <ReviewField label="Full Name" value={data.fullName} span />
          <ReviewField label="Date of Birth" value={data.dob} />
          <ReviewField label="Age" value={data.age ? `${data.age} years` : ""} />
          <ReviewField label="Sex" value={data.sex === "F" ? "Female" : data.sex === "M" ? "Male" : data.sex} />
          <ReviewField label="Village / Zone" value={data.village} />
          <ReviewField label="Phone Number" value={data.phone ? `+223 ${data.phone}` : "Not provided"} />
        </div>
      </div>

      {/* Section: Emergency */}
      <div className="bg-slate-50 rounded-2xl border border-slate-100 p-5">
        <p className="text-xs font-bold uppercase tracking-widest text-teal-600 mb-4">Emergency Contact</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
          <ReviewField label="Contact Name" value={data.emergencyName} />
          <ReviewField label="Relationship" value={data.emergencyRelation} />
          <ReviewField label="Emergency Phone" value={data.emergencyPhone ? `+223 ${data.emergencyPhone}` : ""} />
        </div>
      </div>

      {/* Section: Medical */}
      <div className="bg-slate-50 rounded-2xl border border-slate-100 p-5">
        <p className="text-xs font-bold uppercase tracking-widest text-teal-600 mb-4">Medical History</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
          <ReviewField label="Blood Type" value={data.bloodType} />
          <ReviewField label="Pregnancy Status" value={data.pregnancyStatus.replace(/-/g, " ")} />
          <ReviewField label="Known Allergies" value={data.allergies} span />
          <ReviewField label="Current Medications" value={data.medications} span />
          <ReviewField label="Vaccination Notes" value={data.vaccinations} span />
        </div>
        {data.conditions.length > 0 && (
          <div className="mt-3">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1.5">Chronic Conditions</p>
            <div className="flex flex-wrap gap-1.5">
              {data.conditions.map((c) => (
                <span key={c} className="text-xs bg-teal-100 text-teal-800 font-medium px-2.5 py-1 rounded-xl">{c}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
        ...
        <p className="text-xs text-amber-800 leading-relaxed">
          By submitting, you confirm that verbal consent has been obtained from the patient or their guardian and that all information is accurate to the best of your knowledge.
        </p>
      </div>
    </div>
  );
...
function validateStep(step: number, data: FormData): Partial<Record<keyof FormData, string>> {
  ...
  if (step === 1) {
    ...
    if (!data.emergencyName.trim()) e.emergencyName = "Emergency contact name is required";
    if (!data.emergencyRelation) e.emergencyRelation = "Please select a relationship";
    if (!data.emergencyPhone.trim()) e.emergencyPhone = "Emergency phone number is required";
  }
  return e;
}
...
export default function NewPatientPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto flex flex-col gap-6 pb-10">
      ...
      {/* ── Page header ── */}
      <div>
        <h1 className="font-display text-2xl lg:text-3xl text-teal-950">New Patient Registration</h1>
        <p className="text-sm text-slate-400 mt-0.5">Complete all required fields. Record is saved locally until synced.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm px-6 py-5">
        ...
        <p className="text-[11px] text-slate-400 mt-1.5 text-right">Step {step} of 3</p>
      </div>

      {/* ── Form card ── */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm px-6 py-7">
        <div className="mb-6 pb-5 border-b border-slate-100">
          <h2 className="font-semibold text-slate-800 text-base">{STEPS[step - 1].title}</h2>
          <p className="text-xs text-slate-400 mt-0.5">{STEPS[step - 1].sub}</p>
        </div>

        {step === 1 && <Step1 data={data} update={update as (k: keyof FormData, v: string) => void} errors={errors} />}
