...
const Icon = {
  ...
  trendUp: (
    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={2} className="w-3 h-3">
      <path strokeLinecap="round" strokeLinejoin="round" d="M2 11l4-4 3 3 5-6M14 4h-3M14 4v3" />
    </svg>
  ),
  info: (
...
function WeeklyLineChart() {
  ...
  const W = 640, H = 240, padX = 44, padTop = 20, padBottom = 36;
  const innerW = W - padX * 2, innerH = H - padTop - padBottom;
  const maxV = 2000, minV = 1000;
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" preserveAspectRatio="xMidYMid meet">
        ...
        {grid.map((g) => {
          ...
          return (
            <g key={g}>
              <line x1={padX} y1={y} x2={W - padX} y2={y} stroke="#e2e8f0" strokeWidth="1" strokeDasharray="3 4" />
              <text x={padX - 10} y={y + 3.5} textAnchor="end" className="fill-slate-400" fontSize="10.5">{g >= 1000 ? `${g / 1000}k` : g}</text>
            </g>
          );
        })}
        ...
        {pts.map((p, i) => (
          <g key={p.week}>
            <text x={p.x} y={H - padBottom + 18} textAnchor="middle" className="fill-slate-500" fontSize="10.5" fontWeight={hover === i ? 700 : 500}>{p.week}</text>
            {hover === i && <line x1={p.x} y1={padTop} x2={p.x} y2={H - padBottom} stroke={TEAL} strokeWidth="1" strokeDasharray="3 3" opacity="0.4" />}
            <circle cx={p.x} cy={p.y} r={hover === i ? 6 : 4} fill="white" stroke={TEAL} strokeWidth="2.5" className="transition-all" />
            <rect x={p.x - innerW / (WEEKLY.length - 1) / 2} y={0} width={innerW / (WEEKLY.length - 1)} height={H} fill="transparent" onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)} />
          </g>
        ))}
      </svg>
      {hover !== null && (
        <div className="pointer-events-none absolute -translate-x-1/2 -translate-y-full bg-teal-950 text-white rounded-lg px-3 py-2 shadow-lg" style={{ left: `${(pts[hover].x / W) * 100}%`, top: `${(pts[hover].y / H) * 100}%`, marginTop: "-10px" }}>
          <p className="text-[10px] uppercase tracking-wide text-teal-300 font-semibold whitespace-nowrap">{pts[hover].label} · {pts[hover].week}</p>
          <p className="font-display text-lg leading-none mt-0.5">{pts[hover].visits.toLocaleString()} <span className="text-xs font-sans text-teal-200">visits</span></p>
        </div>
      )}
    </div>
  );
      ...
      {DIAGNOSES.map((d, i) => (
        ...
        >
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs font-semibold text-slate-500 tabular-nums ml-2">{d.count}</span>
          </div>
          <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                ...
                background: hover === i ? TEAL : `linear-gradient(90deg, ${TEAL}, ${TEAL_SOFT})`,
              }}
            />
          </div>
        </div>
        ...
        <svg viewBox="0 0 180 180" className="w-44 h-44">
          {arcs.map((a, i) => (
            <path
              ...
              style={{ opacity: hover === null || hover === i ? 1 : 0.35, transformOrigin: "90px 90px", transform: hover === i ? "scale(1.04)" : "scale(1)" }}
              onMouseEnter={() => setHover(i)}
              onMouseLeave={() => setHover(null)}
            />
          ))}
          <text x="90" y="84" textAnchor="middle" className="fill-teal-950 font-display" fontSize="26">
            {hover !== null ? arcs[hover].count : total}
          </text>
          <text x="90" y="102" textAnchor="middle" className="fill-slate-400" fontSize="10" fontWeight={600}>
      ...
      <div className="space-y-2 min-w-[150px]">
        {arcs.map((a, i) => (
          <button
            ...
            onMouseEnter={() => setHover(i)}
            onMouseLeave={() => setHover(null)}
            className={`w-full flex items-center gap-2.5 text-left rounded-lg px-2 py-1 transition-colors ${hover === i ? "bg-slate-50" : ""}`}
          >
            <span className="w-2.5 h-2.5 rounded-sm flex-shrink-0" style={{ background: URGENCY_COLORS[a.level] }} />
            <span className="text-sm text-slate-600 flex-1">{a.level}</span>
            <span className="text-sm font-semibold text-slate-700 tabular-nums">{Math.round(a.frac * 100)}%</span>
          </button>
        ))}
      </div>
    </div>
    ...
    <div>
      <div className="overflow-x-auto">
        <div className="min-w-[420px]">
          ...
          <div className="grid grid-cols-[92px_repeat(7,1fr)] gap-1.5 mb-1.5">
            <div />
            {DAYS.map((d) => (
              <div key={d} className="text-[10px] font-semibold text-slate-400 text-center uppercase tracking-wide">{d}</div>
            ))}
          </div>
          {/* Rows */}
          {VILLAGES.map((village, vi) => (
            <div key={village} className="grid grid-cols-[92px_repeat(7,1fr)] gap-1.5 mb-1.5">
              <div className="text-xs font-medium text-slate-600 flex items-center truncate pr-1">{village}</div>
              {DAYS.map((_, di) => {
                ...
                return (
                  <div
                    ...
                    onMouseLeave={() => setHover(null)}
                    className={`relative aspect-square rounded-md flex items-center justify-center text-[10px] font-semibold cursor-pointer transition-all ${textColor(val)} ${active ? "ring-2 ring-teal-600 ring-offset-1 z-10" : ""}`}
                  ...
                  >
                    ...
                    {active && (
                      <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 bg-teal-950 text-white rounded-lg px-2.5 py-1.5 shadow-lg whitespace-nowrap z-20">
                        <p className="text-[10px] text-teal-300 font-semibold">{VILLAGES[vi]} · {DAYS[di]}</p>
      ...
      <div className="flex items-center gap-2 mt-4 justify-end">
        ...
        <div className="flex gap-0.5">
          {["#f0fdfa", "#ccfbf1", "#5eead4", "#2dd4bf", "#14b8a6", "#0d9488"].map((c) => (
...
// ── Card wrapper ──────────────────────────────────────────────────────────────────
function ChartCard({ title, subtitle, children, span, badge }: { title: string; subtitle?: string; children: React.ReactNode; span?: boolean; badge?: React.ReactNode }) {
  return (
    <div className={`bg-white rounded-2xl border border-slate-100 p-5 lg:p-6 ${span ? "lg:col-span-2" : ""}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-display text-2xl lg:text-3xl text-teal-950">Analytics &amp; Reports</h1>
          <p className="text-sm text-slate-500 mt-0.5">Kayes Health District · trends across all clinics</p>
        </div>
          ...
          <div className="flex items-center gap-1 bg-slate-100 rounded-xl p-1">
            {(["8w", "6m", "1y"] as const).map((r) => (
              <button
                key={r}
                onClick={() => setRange(r)}
                className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ${range === r ? "bg-white text-teal-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        >
          ...
          <div className="flex items-center gap-2 mt-4 pt-4 border-t border-slate-100">
            <span className="w-3 h-0.5 rounded-full" style={{ background: TEAL }} />
            <span className="text-xs text-slate-500">Weekly patient visits</span>
            <span className="text-xs text-slate-400 ml-auto">Peak: Wk 34, 1,856 visits</span>
          </div>
        </ChartCard>

        {/* Bar chart — common diagnoses */}
        <ChartCard title="Most Common Diagnoses" subtitle="By recorded visits this period">
          <DiagnosesBarChart />
        </ChartCard>
        ...
        <ChartCard
          ...
          span
          badge={<span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">This week</span>}
        >
          <VillageHeatmap />
        </ChartCard>
      </div>
