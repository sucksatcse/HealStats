...
const Icon = {
  ...
  newPatient: (
    <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.7} className="w-4.5 h-4.5">
      <path strokeLinecap="round" strokeLinejoin="round" d="M16 11a4 4 0 11-8 0 4 4 0 018 0zM12 7v8M8 11h8M2 19v-2a4 4 0 014-4h1" />
    </svg>
  ),
  sync: (
    <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.7} className="w-4.5 h-4.5">
...
const STATUS_LABELS: Record<string, { label: string; cls: string }> = {
  "follow-up": { label: "Follow-up", cls: "bg-blue-50 text-blue-600 border-blue-200" },
  chronic: { label: "Chronic", cls: "bg-violet-50 text-violet-600 border-violet-200" },
  antenatal: { label: "Antenatal", cls: "bg-pink-50 text-pink-600 border-pink-200" },
...
const NAV_ITEMS = [
  ...
  { id: "emergency",      label: "Emergency Report", icon: (
    <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.7} className="w-4.5 h-4.5">
  ...
  { id: "triage-queue",   label: "Triage Queue", icon: (
    <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.7} className="w-4.5 h-4.5">
      <path strokeLinecap="round" strokeLinejoin="round" d="M2 10h3l1.5-5 3 11 2.5-7 1.5 2h3.5" />
    </svg>
  ) },
];

const STATS = [
  { label: "Patients Today", labelBn: "আজকের রোগী", value: "23", sub: "+4 since morning", subBn: "সকাল থেকে +৪", accent: "text-teal-700" },
  { label: "Total Records", labelBn: "মোট রেকর্ড", value: "1,048", sub: "across all visits", subBn: "সব ভিজিট জুড়ে", accent: "text-slate-700" },
  { label: "Pending Sync", labelBn: "সিঙ্ক বাকি", value: "14", sub: "queued locally", subBn: "স্থানীয়ভাবে সারিবদ্ধ", accent: "text-amber-700" },
  { label: "This Month", labelBn: "এই মাসে", value: "187", sub: "visits recorded", subBn: "ভিজিট রেকর্ড হয়েছে", accent: "text-violet-700" },
];
...
const COPY = {
  en: {
    ...
    connectedSub: (last: string) => `14 records queued locally · Last synced: ${last}`,
    offlineTitle: "Offline mode active",
    offlineSub: "14 records queued · Will sync automatically when connection is restored",
    syncNow: "Sync Now",
    syncing: "Syncing…",
    ...
    showing: (n: number, q: string) => `Showing ${n} result${n !== 1 ? "s" : ""} for "${q}"`,
  ...
  bn: {
    ...
    connectedSub: (last: string) => `স্থানীয়ভাবে ১৪টি রেকর্ড সারিবদ্ধ · সর্বশেষ সিঙ্ক: ${last}`,
...
export default function DashboardPage({ onLogout }: DashboardPageProps) {
  ...
  const filteredPatients = RECENT_PATIENTS.filter((p) =>
    searchValue === "" ||
  ...
  return (
    <div className={`${lang === "bn" ? "lang-bn" : ""} flex h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden font-[Work_Sans,system-ui,sans-serif] transition-colors`}>
      ...
      >
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          <p className="text-[10px] font-bold uppercase tracking-widest text-teal-500 px-3 mb-3">{t.navigation}</p>
          {NAV_ITEMS.map(({ id, label, icon }) => {
            ...
            return (
              ...
              >
                ...
                {id === "emergency" && (
                  <span className={`ml-auto text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-full ${active ? "bg-white/25 text-white" : "bg-red-500 text-white"}`}>
                    SOS
                  </span>
                )}
                {id === "triage-queue" && (
                  <span className="ml-auto flex items-center gap-1 text-[10px] font-bold bg-red-500 text-white px-1.5 py-0.5 rounded-full">
                    <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />{lang === "bn" ? toBn(8) : 8}
                  </span>
                )}
              </button>
            );
        ...
        <div className="px-3 py-4 border-t border-teal-800">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-teal-800/60 transition-colors group">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white truncate">{lang === "bn" ? "সিস্টার আমারা দিয়ালো" : "Sr. Amara Diallo"}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className={`flex-1 overflow-y-auto px-4 lg:px-8 py-6 ${activeNav === "patients" ? "flex flex-col min-h-0" : "space-y-6"}`}>
          ...
          {!["patients","new-patient","vitals","triage","patient-detail","digitize","sync","emergency","triage-queue"].includes(activeNav) && (<>
          ...
          <div className="flex items-start justify-between flex-wrap gap-3">
            ...
            <div className="text-right">
              <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold">{t.clinic}</p>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              {isOnline ? (
                <>
                  <p className="text-sm font-semibold text-emerald-900">{t.connected}</p>
                ...
                <>
                  <p className="text-sm font-semibold text-amber-900">{t.offlineTitle}</p>
          ...
          {/* Quick actions */}
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">{t.quickActions}</p>
            <div className="flex flex-wrap gap-3">
              {[
                {
                  label: t.newPatient,
                  icon: Icon.newPatient,
                  color: "bg-teal-600 hover:bg-teal-700 text-white shadow-md shadow-teal-600/25",
                  action: () => setActiveNav("new-patient"),
                },
                {
                  label: t.searchPatient,
                  icon: Icon.search,
                  color: "bg-white hover:bg-teal-50 text-teal-700 border border-teal-200",
                  action: () => { const el = document.querySelector("input") as HTMLInputElement; el?.focus(); },
                },
                {
                  label: t.syncNow,
                  icon: Icon.sync,
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {STATS.map(({ label, labelBn, value, sub, subBn, accent }) => (
              <div key={label} className="bg-white rounded-2xl border border-slate-100 p-5 hover:shadow-md transition-shadow">
                <p className="text-xs text-slate-400 font-medium mb-2">{lang === "bn" ? labelBn : label}</p>
                <p className={`font-display text-3xl ${accent} mb-1`}>{lang === "bn" ? toBn(value) : value}</p>
                <p className="text-xs text-slate-400">{lang === "bn" ? subBn : sub}</p>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="font-semibold text-slate-800 text-base">{t.recentTitle}</h2>
                {searchValue && (
            ...
            {filteredPatients.length === 0 ? (
              <div className="bg-white rounded-2xl border border-slate-100 py-16 text-center">
                ...
                <p className="text-sm font-medium text-slate-500">{t.noMatch(searchValue)}</p>
                ...
                {filteredPatients.map((p) => (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3 mb-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold flex-shrink-0 ${p.color}`}>
                        {p.initials}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-slate-400 mt-0.5">
                          {lang === "bn" ? toBn(p.age) : p.age} {t.yrs} · {p.gender === "F" ? t.female : t.male} · {p.id}
                        </p>
                      </div>
                      <span
                        className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border flex-shrink-0 ${STATUS_LABELS[p.status].cls}`}
                      >
                        {lang === "bn" ? STATUS_LABELS_BN[p.status] : STATUS_LABELS[p.status].label}
                      </span>
                    </div>

                    {/* Diagnosis */}
                    <div className="bg-slate-50 rounded-xl px-3 py-2.5 mb-4">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-0.5">{t.diagnosis}</p>
                      <p className="text-xs font-medium text-slate-700 leading-snug">{lang === "bn" ? DIAGNOSIS_BN[p.id] ?? p.diagnosis : p.diagnosis}</p>
                    </div>
