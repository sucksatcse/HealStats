...
const PARTICLES = [
  { left: "12%", color: "bg-teal-400", size: "w-2 h-2", delay: "0.5s", spin: "220deg" },
  { left: "24%", color: "bg-amber-300", size: "w-1.5 h-1.5", delay: "0.65s", spin: "-180deg" },
  { left: "38%", color: "bg-teal-500", size: "w-2 h-2 rounded-sm", delay: "0.55s", spin: "160deg" },
  { left: "50%", color: "bg-emerald-300", size: "w-1.5 h-1.5", delay: "0.75s", spin: "-240deg" },
  { left: "62%", color: "bg-teal-300", size: "w-2 h-2 rounded-sm", delay: "0.6s", spin: "200deg" },
  { left: "76%", color: "bg-amber-300", size: "w-1.5 h-1.5", delay: "0.7s", spin: "-160deg" },
  { left: "88%", color: "bg-teal-400", size: "w-2 h-2", delay: "0.5s", spin: "260deg" },
];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-50/60 via-white to-white flex items-center justify-center px-4 font-[Work_Sans,system-ui,sans-serif]">
      <div key={run} className="w-full max-w-md text-center">
          ...
          <div className="pointer-events-none absolute -inset-x-8 -top-4 bottom-0">
            {PARTICLES.map((p, i) => (
              <span
                ...
                style={{ left: p.left, animationDelay: p.delay, ["--spin" as string]: p.spin }}
              />
            ))}
          </div>
        ...
        {/* Copy */}
        <h1 className="font-display text-3xl text-teal-950 mb-2 animate-slide-up" style={{ animationDelay: "0.5s" }}>
          Record Saved Successfully
        </h1>
        <p className="text-slate-500 leading-relaxed mb-1 animate-slide-up" style={{ animationDelay: "0.6s" }}>
        ...
        {/* Actions */}
        <div className="flex flex-col sm:flex-row gap-3 justify-center animate-slide-up" style={{ animationDelay: "0.75s" }}>
          <button
            onClick={onDone}
          ...
          <button
            onClick={() => (onSaveAnother ? onSaveAnother() : setRun((r) => r + 1))}
