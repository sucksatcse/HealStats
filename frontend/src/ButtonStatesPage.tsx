...
const STATES: {
  ...
  },
  {
    ...
    classes: {
      primary: "bg-teal-700 text-white shadow-md shadow-teal-700/30 scale-[1.03]",
  ...
  },
  {
    ...
    classes: {
      ...
      secondary: "bg-teal-100 text-teal-900 border border-teal-500 scale-[0.97]",
    },
    content: () => <>{plusIcon}New Patient</>,
  },
  {
    ...
    classes: {
      primary: "bg-teal-600 text-white opacity-55 cursor-not-allowed shadow-none",
      secondary: "bg-white text-slate-400 border border-slate-200 cursor-not-allowed",
    },
    content: () => <>{plusIcon}New Patient</>,
    disabled: true,
  },
      ...
      {STATES.map((s) => (
        <div key={s.id} className="flex flex-col items-center text-center gap-3">
          <div className="h-20 flex items-center justify-center w-full">
            <button className={`${base} ${s.classes[variant]}`} disabled={s.disabled} tabIndex={-1}>
              {s.content(variant)}
            </button>
          </div>
          <div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-[Work_Sans,system-ui,sans-serif]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {/* Header */}
        <p className="text-xs font-bold tracking-[0.15em] uppercase text-teal-600 mb-3">Micro-interactions</p>
        ...
        <p className="text-lg text-slate-500 max-w-2xl leading-relaxed mb-10">
          Every button in HealthStats moves through the same five states. Consistent, legible feedback
          matters most when a health worker is tapping quickly between patients.
        </p>
        ...
        <section className="bg-white rounded-2xl border border-slate-100 p-6 lg:p-8 mb-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-xs text-slate-400">— main call to action, one per view</span>
          </div>
        ...
        <section className="bg-white rounded-2xl border border-slate-100 p-6 lg:p-8 mb-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-xs text-slate-400">— supporting actions, sits beside the primary</span>
          </div>
        ...
        {/* Annotations */}
        <section className="bg-white rounded-2xl border border-slate-100 p-6 lg:p-8 mb-6">
          <h2 className="font-semibold text-slate-800 mb-5">State annotations</h2>
          <div className="space-y-3">
            {STATES.map((s) => (
              <div key={s.id} className="flex items-start gap-3 py-3 border-b border-slate-100 last:border-0">
        ...
        <section className="bg-gradient-to-br from-teal-50 to-white rounded-2xl border border-teal-100 p-6 lg:p-8">
          ...
          <p className="text-sm text-slate-500 mb-6">
            Hover, press, and click — clicking triggers the loading state for ~2 seconds.
          </p>
