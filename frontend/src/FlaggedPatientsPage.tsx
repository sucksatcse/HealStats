...
const INITIAL: Flagged[] = [
  { id: "PT-00376", name: "Oumar Coulibaly", age: 8, gender: "M", village: "Diamou", score: 94, level: "Critical", symptoms: "High fever (40.1°C), rapid breathing, chest indrawing", vitals: "RR 48 · SpO₂ 89%", flaggedAt: "4 min ago", initials: "OC", color: "bg-sky-100 text-sky-700" },
  { id: "PT-00331", name: "Modibo Keïta", age: 70, gender: "M", village: "Yélimané", score: 91, level: "Critical", symptoms: "Chest pain radiating to arm, sweating, shortness of breath", vitals: "BP 178/104 · HR 112", flaggedAt: "12 min ago", initials: "MK", color: "bg-emerald-100 text-emerald-700" },
  { id: "PT-00251", name: "Hawa Camara", age: 41, gender: "F", village: "Kéniéba", score: 88, level: "Critical", symptoms: "Severe abdominal pain, vomiting blood, dizziness", vitals: "BP 88/56 · HR 128", flaggedAt: "26 min ago", initials: "HC", color: "bg-rose-100 text-rose-700" },
  { id: "PT-00412", name: "Mariama Kouyaté", age: 34, gender: "F", village: "Diamou", score: 79, level: "High", symptoms: "Persistent high fever, chills, confirmed malaria smear", vitals: "Temp 39.4°C · HR 104", flaggedAt: "38 min ago", initials: "MK", color: "bg-violet-100 text-violet-700" },
  { id: "PT-00365", name: "Kadiatou Baldé", age: 61, gender: "F", village: "Yélimané", score: 74, level: "High", symptoms: "Uncontrolled hypertension, blurred vision, headache", vitals: "BP 192/110", flaggedAt: "1 hr ago", initials: "KB", color: "bg-amber-100 text-amber-700" },
  { id: "PT-00281", name: "Yacouba Sidibé", age: 56, gender: "M", village: "Yélimané", score: 71, level: "High", symptoms: "Diabetic, non-healing foot ulcer with early necrosis", vitals: "Glucose 21 mmol/L", flaggedAt: "1 hr ago", initials: "YS", color: "bg-lime-100 text-lime-700" },
  { id: "PT-00318", name: "Rokia Cissé", age: 3, gender: "F", village: "Diamou", score: 68, level: "High", symptoms: "Dehydration from acute diarrhoea, lethargy, sunken eyes", vitals: "Weight -8% · HR 138", flaggedAt: "2 hrs ago", initials: "RC", color: "bg-fuchsia-100 text-fuchsia-700" },
  { id: "PT-00292", name: "Assitan Doumbia", age: 29, gender: "F", village: "Kéniéba", score: 61, level: "Moderate", symptoms: "3rd-trimester pregnancy, elevated BP, mild oedema", vitals: "BP 148/94 · 34 wks", flaggedAt: "3 hrs ago", initials: "AD", color: "bg-orange-100 text-orange-700" },
  { id: "PT-00240", name: "Adama Fofana", age: 24, gender: "M", village: "Yélimané", score: 57, level: "Moderate", symptoms: "Deep laceration, signs of local infection, low-grade fever", vitals: "Temp 38.1°C", flaggedAt: "4 hrs ago", initials: "AF", color: "bg-teal-100 text-teal-700" },
];

const DOCTORS = ["Dr. Priya Suresh", "Dr. Fanta Diallo", "Dr. Kwame Osei", "Dr. Lina Haddad"];
...
// ── Assign dropdown ──────────────────────────────────────────────────────────────
function AssignMenu({ assigned, onAssign }: { assigned: string | null; onAssign: (doctor: string) => void }) {
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-20 w-52 bg-white rounded-xl border border-slate-100 shadow-xl py-1.5 animate-slide-up">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 px-3.5 py-1.5">Available doctors</p>
            {DOCTORS.map((d) => (
              <button
                key={d}
      ...
      <div className="rounded-2xl border border-red-200 bg-gradient-to-r from-red-50 to-orange-50 px-5 py-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-red-700/80 mt-0.5">
            {INITIAL.length} patients flagged by on-device AI triage · <span className="font-semibold">{criticalCount} critical</span> · {unassignedCount} awaiting assignment
          </p>
        </div>
          ...
          <table className="w-full min-w-[920px] text-left">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                {["Patient", "Urgency Score", "Symptoms Summary", "Flagged"].map((col) => (
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className={`group hover:bg-slate-50/60 transition-colors border-l-4 ${assigned ? "border-l-emerald-400 opacity-80" : meta.ring}`}>
                    {/* Patient */}
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-slate-400">{r.age} yrs · {r.gender === "F" ? "F" : "M"} · {r.village} · {r.id}</p>
                        </div>
                      </div>
                    </td>
                    {/* Urgency score */}
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3 w-40">
                        <div className="flex items-baseline gap-1 w-12 flex-shrink-0">
                          <span className={`font-display text-2xl leading-none ${meta.scoreText}`}>{r.score}</span>
                        </div>
                        <div className="flex-1">
                          ...
                          <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${meta.bar}`} style={{ width: `${r.score}%` }} />
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 max-w-xs">
                      <p className="text-sm text-slate-700 leading-snug">{r.symptoms}</p>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end">
                        <AssignMenu assigned={assigned || null} onAssign={(d) => assign(r.id, r.name, d)} />
                      </div>
                    </td>
                  </tr>
                );
