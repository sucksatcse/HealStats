      ...
      <div className="max-w-2xl mx-auto py-6">
        <div className="bg-white rounded-3xl border border-red-100 shadow-xl shadow-red-900/5 overflow-hidden">
          <div className="bg-gradient-to-br from-red-600 to-orange-600 px-8 py-10 text-center">
            ...
            <h2 className="font-display text-3xl text-white">Emergency report dispatched</h2>
            <p className="text-red-50 text-sm mt-2 max-w-md mx-auto leading-relaxed">
              Queued locally and broadcasting to the District Coordination Centre. It will resend automatically until acknowledged.
            </p>
          </div>
          <div className="px-8 py-6 space-y-3">
            {[
              ...
              <div key={k} className="flex items-center justify-between text-sm border-b border-slate-100 pb-3 last:border-0 last:pb-0">
          ...
          <div className="px-8 pb-8">
            <button
              onClick={() => {
                setSubmitted(false);
                setIncidentType(""); setLocation(null); setAffected("");
                setSeverity("critical"); setNotes(""); setPhotos([]);
              }}
      ...
      <div className="rounded-2xl bg-gradient-to-r from-red-600 to-orange-600 px-6 py-5 mb-6 flex items-center gap-4 shadow-lg shadow-red-900/10 relative overflow-hidden">
        ...
        <div className="flex-1 min-w-0 relative">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold uppercase tracking-widest text-white bg-white/20 rounded-full px-2 py-0.5">Disaster Mode</span>
            <span className="flex items-center gap-1 text-[11px] text-red-50 font-medium">{Icon.clock} {now}</span>
          </div>
          <h1 className="font-display text-2xl text-white mt-1.5 leading-tight">Emergency Incident Report</h1>
        </div>
      </div>

      <p className="text-sm text-slate-500 mb-6 leading-relaxed">
        Report a mass-casualty or emergency event. Fields marked <span className="text-red-600 font-semibold">*</span> are required to dispatch. Reports are stored offline and broadcast the moment any signal returns.
      </p>
      ...
      <div className="space-y-6">
          ...
          <div className="relative">
            <select
              ...
              className={`w-full appearance-none rounded-xl border bg-white pl-4 pr-10 py-3.5 text-sm font-medium transition-all focus:outline-none focus:ring-4 focus:ring-red-500/15 focus:border-red-400 ${
                incidentType ? "border-slate-300 text-slate-800" : "border-slate-200 text-slate-400"
              }`}
            >
              <option value="" disabled>Select incident type…</option>
              {INCIDENT_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
            <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">{Icon.chevron}</span>
          </div>
        </Field>
          ...
          <div className="rounded-2xl border border-slate-200 overflow-hidden bg-slate-50">
            <div className="relative aspect-[16/9] bg-gradient-to-br from-sky-50 to-teal-50">
              <svg viewBox="0 0 100 56" className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
                ...
                {/* delta landmass */}
                <path d="M0 0 H100 V30 C88 33 80 30 70 36 C60 42 52 40 44 46 C36 52 24 50 14 54 C8 56 4 54 0 56 Z" fill="#dcefe3" />
                {/* rivers */}
                <path d="M30 0 C34 14 26 22 32 34 C36 44 30 50 34 56" stroke="#bfe3ea" strokeWidth="1.6" fill="none" />
                <path d="M62 0 C58 12 66 20 60 30 C56 38 62 46 58 56" stroke="#bfe3ea" strokeWidth="1.6" fill="none" />
              </svg>
              ...
              {MAP_PINS.map((p) => {
                ...
                return (
                  ...
                  >
                    {active && <span className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-red-500/25 animate-ping" />}
                    <span className={`relative block transition-all ${active ? "text-red-600 scale-125" : "text-slate-400 group-hover:text-red-400 group-hover:scale-110"}`}>
                    ...
                    <span className={`absolute left-1/2 -translate-x-1/2 mt-0.5 whitespace-nowrap text-[10px] font-semibold px-1.5 py-0.5 rounded transition-opacity ${
                      active ? "bg-red-600 text-white opacity-100" : "bg-white/90 text-slate-500 opacity-0 group-hover:opacity-100"
                    }`}>
                      {p.label}
                    </span>
                  </button>
            ...
            <div className="px-4 py-3 bg-white border-t border-slate-100 flex items-center gap-2 text-xs">
              {location ? (
                ...
                <span className="text-slate-400">Tap a pin to mark the incident location</span>
              )}
            </div>
          </div>
        </Field>
            ...
            <div className="space-y-2">
              {SEVERITY_LEVELS.map((s) => (
                <label key={s.id} className="block cursor-pointer">
                  ...
                  />
                  <div className={`flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 transition-all peer-focus-visible:ring-4 ${s.ring}`}>
                    <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${s.dot}`} />
                    <div className="min-w-0">
                      <p className={`text-sm font-semibold text-slate-700 ${s.text}`}>{s.label}</p>
        ...
        <Field label="Photo Evidence" icon={Icon.camera}>
          <label className="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 hover:border-red-300 hover:bg-red-50/40 transition-colors py-8 cursor-pointer text-center">
            <span className="w-12 h-12 rounded-2xl bg-white shadow-sm flex items-center justify-center text-red-500">{Icon.camera}</span>
            <span className="text-sm font-semibold text-slate-600">Tap to add photos</span>
            <span className="text-xs text-slate-400">Compressed and stored offline · up to 6 images</span>
            <input type="file" accept="image/*" multiple capture="environment" className="hidden" onChange={(e) => handleFiles(e.target.files)} />
          </label>
          {photos.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-3">
              {photos.map((name, i) => (
                <div key={i} className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg pl-2.5 pr-1.5 py-1.5">
                  <span className="text-red-500">{Icon.camera}</span>
                  <span className="text-xs font-medium text-slate-600 max-w-[120px] truncate">{name}</span>
                  <button
                    type="button"
