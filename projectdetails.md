...
- **Prepared coordinate extension (2026-09-08, NOT applied)**: `20260908000000_add_clinic_coordinates.sql` adds nullable `latitude` / `longitude` double precision, paired-null and Bangladesh-envelope range checks (latitude 20.5–26.7, longitude 88–92.7). No default inferred positions, no backfill or RLS changes. Admin map detects missing columns and falls back to unpinned clinic listings with editing disabled.
...
- **Isolation (2026-09-08)**: Playwright always builds its own preview with `VITE_SUPABASE_URL=https://healstats-e2e.invalid` and a non-secret test key, blocks service workers, and disables preview reuse. Authenticated tests mock REST/Auth/Edge Function requests and block unhandled writes. Use pnpm 9 to match CI; 27 E2E and 17 unit tests pass. These are mocked UI tests, not live RLS or clinical-write verification.
- **Static checks**: `tsc --noEmit` and `vite build` before merging.
- **Map checks (2026-09-08)**: `test:unit` now includes Node proxy/backfill tests after Vitest (96 + 28 passing). Playwright has 53 passing tests, including actual Leaflet projection/zoom/pan, search, fallback, editor and mobile checks with all map tiles/geocoding/DB mocked. The backfill is never invoked in lookup/apply mode by ordinary build/test commands.
...
## 23. Change Log
- **2026-09-08 — Map replacement**: Admin map now uses Leaflet/react-leaflet (OSM/CARTO) and stored coordinates rather than district SVG lookup. Added paginated metric reads, fail-closed metric errors, admin-authorized online `saveClinic`, bilingual responsive editor/search and an authenticated `/api/geocode` Node proxy. The proxy reuses Supabase identity/staff authorization through read-only REST, never a browser service-role key. One server process plus persistent disk lock/cache enforces Nominatim app-wide rate limits; Vite integrates dev middleware only, production requires an explicit reverse proxy. A reviewed-only one-time backfill tool is prepared; neither it nor the coordinate migration was applied. `/api/` is excluded from service-worker caching. Public coverage component unchanged. Deployment details in README; these supersede D-008's SVG-coordinate approach for the admin map only.
- **2026-09-08**: Fixed Phase-merge QA regressions: corrected login/signup/logout E2E navigation, prevented admin-login redirects from discarding denial errors, required resolved staff profiles before protected rendering, and ignored superseded profile results after sign-out. Removed login-time metadata-based staff creation and synthetic profile fallback; missing staff records require account setup rather than granting access. Existing signup provisioning and email-based staff lookup remain unchanged. Mobile navbar auth actions now use the drawer below 1280px, with compact branding below 400px. TypeScript, production preview build, 17 unit tests, and 27 E2E tests pass. No schema/migration execution; existing build-size warnings and production authorization hardening remain outstanding.
